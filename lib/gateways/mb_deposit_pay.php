<?php
	...
	$business = get_option('shipme_moneybookers_email');
	...
	$cancel_url 	= shipme_get_payments_page_url('deposit');
	$response_url 	= get_bloginfo('siteurl').'/?s_action=mb_deposit_response';
	$ccnt_url		= shipme_get_payments_page_url();
...
<center><h3><?php _e('Please wait, your order is being processed...', 'shipme'); ?></h3></center>

	
    <form name="form_mb" action="https://www.moneybookers.com/app/payment.pl">
    <input type="hidden" name="pay_to_email" value="<?php echo get_option('shipme_moneybookers_email'); ?>">
