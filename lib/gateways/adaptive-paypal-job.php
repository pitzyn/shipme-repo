<?php
 ...
 //------------------------------------------------------------------------
	...
	$receiver_user_id = $bid->uid;			
 	...
 	$fee = get_option('shipme_fee_after_paid');
	if(empty($fee)) { die('ERROR. Please specify an amount for the percent taken by admin for each job.'); }
 ...
 //-------------------------------------------------------------------------	
	...
	$receiver1 	= get_option('ShipMe_paypal_email'); if(empty($receiver1)) { die('ERROR. The site admin has no paypal email defined.'); }
	$amount1 	= ShipMe_formats_special($adminfee, 2);
	...
	$amount2 	= ShipMe_formats_special($total-$adminfee, 2); 
 
	$currency 	= get_option('ShipMe_currency');  
 	$enasdbx 	= get_option('ShipMe_paypal_enable_sdbx');
